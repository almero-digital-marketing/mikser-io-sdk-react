...
A standalone mikser project. Run it once, point all three React examples at it. They share one catalog, three rendering shapes.
...
| `schemas/*.js` | Zod schemas — one per layout (`page`, `article`, `product`, `landing`, `slot`). `mikser-io-plugin-schemas` validates every loaded entity against the schema that matches its `meta.layout`, and emits `entities.d.ts` for the React apps to import as types. |
...
| Layout | Where it appears | Required meta fields | React view |
|---|---|---|---|
...
| `slot` | `slots/*.md` | + starts_at, duration_minutes, available | (no view — surfaces inside `BookingForm.jsx` island only) |

The body is plain Markdown; mikser's `render-markdown` plugin converts it to HTML for the catalog. The React apps consume that HTML via `document.content` and render it with `dangerouslySetInnerHTML`.
...
Then in each of the React example projects:
...
1. Open all three React examples in your browser at once.
...
- `pure-spa` re-renders `ProductView.jsx` — button enables, label flips
...
Same `useDocument` hook on the client. Same SSE event from the server. Same file on disk as the source of truth.
...
The `hybrid-ssg` build script (`scripts/generate-routes.mjs`) reads from the api at build time, then doesn't talk to the server again. The editor side talks to the api at runtime and stays subscribed.
...
Try adding a new file under `documents/en/articles/` while the examples are running. It appears at the top of `ArticleIndex.jsx` in `pure-spa` and `hybrid-ssg`'s editor — no manual refresh.
...
1. **Validates each entity's `meta` against the matching schema** as it's loaded. The demo runs in `onError: 'warn'` mode, so a typo (e.g. dropping `author` from an article) shows up as a server log line — the entity still appears in the catalog and the React apps still render it. Flip to `onError: 'fail'` in `mikser.config.js` to make the build exit non-zero on the first invalid entity.
2. **Emits `entities.d.ts`** at the project root after every build, exposing one `XxxMeta` type per layout plus a `MetaByLayout<L>` helper. The React apps import from this file to get typed `useDocument` results:
