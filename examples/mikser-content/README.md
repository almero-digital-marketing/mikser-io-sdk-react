...
| `schemas/*.js` | Zod schemas — one per layout (`page`, `article`, `product`, `landing`, `slot`). `mikser-io-schemas` validates every loaded entity against the schema that matches its `meta.layout`, and emits `entities.d.ts` for the React apps to import as types. |
